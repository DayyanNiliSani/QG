...
import { CreateQuestionDto, mapModelToDto, ReadQuestionDto, UpdateQuestionDto } from "src/Infra/Repositories/Question/question.dto";
...
export class QuestionService{
    ...
    async getCategoryQuestions(categoryId: number, skip:number, take:number):Promise<ReadQuestionDto[]>{
        return (await this.questionRepo.getCategoryQuestions(categoryId, skip, take))
            .map(q => mapModelToDto(q))
    }

    async getUserQuestions(userId:number, skip, take:number):Promise<ReadQuestionDto[]>{
        return (await this.questionRepo.getUserQuestions(userId, skip,take))
            .map(q => mapModelToDto(q))
    }
